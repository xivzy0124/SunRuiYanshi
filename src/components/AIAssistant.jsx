...
const WORKFLOW_TEMPLATES = {
  '足底压力': {
    ...
    nodes: [
      ...
      { type: 'process', label: '数据过滤', x: 350, y: 250, config: { operation: '过滤', expression: '剔除无效帧/空值/异常值' } },
      { type: 'transform', label: '字段映射', x: 600, y: 250, config: { method: '映射', mapping: 'sensor_values → pressure_data' } },
      { type: 'process', label: '特征计算', x: 850, y: 250, config: { operation: '计算', expression: '重心/COP/压强分布/平衡比' } },
      { type: 'output', label: '数据入库', x: 1100, y: 250, config: { target: '数据库', url: 't_fusion_health_dataset', format: 'JSON' } },
    ],
    edges: [
      { source: 0, target: 1 },
      { source: 1, target: 2 },
  ...
  '双源融合': {
    ...
    nodes: [
      ...
      { type: 'process', label: '压力过滤', x: 320, y: 150, config: { operation: '过滤', expression: '无效帧/空值/异常值' } },
      { type: 'process', label: '姿态过滤', x: 320, y: 400, config: { operation: '过滤', expression: '置信度/空值/异常坐标' } },
      { type: 'transform', label: '压力映射', x: 560, y: 150, config: { method: '映射', mapping: 'sensor_values → pressure_data' } },
      { type: 'transform', label: '姿态映射', x: 560, y: 400, config: { method: '映射', mapping: 'landmarks → pose_data' } },
      { type: 'process', label: '压力计算', x: 800, y: 150, config: { operation: '计算', expression: '重心/COP/压强分布' } },
      { type: 'process', label: '姿态计算', x: 800, y: 400, config: { operation: '计算', expression: '关节夹角/步态参数' } },
      { type: 'merge', label: '双流融合 JOIN', x: 1040, y: 275, config: { strategy: '并集', key: 'ts (50ms nearest)' } },
      { type: 'output', label: '数据入库', x: 1300, y: 220, config: { target: '数据库', url: 't_fusion_health_dataset' } },
      { type: 'output', label: 'API 发布', x: 1300, y: 340, config: { target: 'API', url: '/api/v1/latest' } },
    ],
    edges: [
      { source: 0, target: 2 },
      { source: 1, target: 3 },
